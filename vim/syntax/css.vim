...
syn region cssFunction contained matchgroup=cssFunctionName start="\<\(rgb\|rgba\|hsl\|hsla\|linear-gradient\|radial-gradient\|clip\|attr\|counter\|rect\)\s*(" end=")" oneline keepend
" Highlighting vendor specific properties is unfinished
"syn region cssFunction contained matchgroup=cssFunctionName start="\<\(-\(webkit\|moz\|ms\|o\)-\)\s*(" end=")" oneline keepend
...
syn match cssColorProp contained "\<background\(-\(attachment\|clip\|color\|image\|origin\|position\|repeat\|size\)\)\=\>"
syn keyword cssColorAttr contained center scroll fixed
syn keyword cssColorAttr contained border-box padding-box content-box
syn keyword cssColorAttr contained contain cover
...
syn match cssBoxProp contained "\<border-\(\(\(top\|right\|bottom\|left\)-\)\=\(width\|color\|style\|radius\)\)\=\>"
syn match cssBoxProp contained "\<box-\(\(sizing\|shadow\)\)\=\>"
syn match cssBoxProp contained "\<\(width\|opacity\|reize\|z-index\|zoom\)\>"
...
syn keyword cssPseudoClassId contained link visited active hover focus before after left right empty target enabled disabled checked selection
syn match cssPseudoClassId contained "\<\(first\|last\|only\)\(-\(line\|letter\|child\|of-type\)\)\=\>"
syn match cssPseudoClassId contained "\<\(nth\)\(-\(child\|of-type\|last-child\|last-of-type\)\)\=\>"
syn match cssPseudoClassId contained "\<\(not\)\>"
